...
class TestRequestValidator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.load_patcher = patch('jwt.load')
        self.load = self.load_patcher.start()
        self.verify_patcher = patch('jwt.verify_signature')
    ...
    def test_authenticate_client_csrf_ok(self):
        ...
        self.client.return_value = MockClient(self.request, KEY)
        with patch('pyramid.session.check_csrf_token') as csrf:
            csrf.return_value = True
            res = self.validator.authenticate_client(self.request)
            assert res is True
            assert self.request.client.client_id == KEY
            assert self.request.client.client_secret == SECRET
